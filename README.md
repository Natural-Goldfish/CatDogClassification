...
The new images and annotation files which are generated by _make_annotation.py_ are saved to ```'data/images'``` and ```'data/annotations'``` for each. The annotation file looks like below, which is a part of _test_annotation.json_. :   
...
```
In the dataset, there are 8004 images for training and 2022 images for testing. The following images are some of the dataset images.</br></br>

...
Honestly, in this project, the original image size doens't need. But I practiced the way to make an annotation file for the other projects kind of _Object Detection_.
The new images and annotation files which are generated by _make_annotation.py_ are saved to ```'data/images'``` and ```'data/annotations'``` for each.
...
```
This is an example of the test_annotation.json. train_annotation.json also has same structure. The following images are some of the dataset images.</br></br>
